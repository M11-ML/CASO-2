...
def reroot_to_depot(route):
    """Rota la ruta para que siempre empiece y termine en el depósito (índice 0)."""
    inner = route[:-1]  # quitar el último (repetición del inicio)
    if inner[0] == 0:
        return route
    depot_pos = inner.index(0)
    rotated = inner[depot_pos:] + inner[:depot_pos]
    return rotated + [0]


def solve_tsp():
    """Resuelve TSP: NN + 2-opt desde todos los nodos de inicio, siempre sale del depósito."""
